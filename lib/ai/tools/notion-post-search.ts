...
export const notionPostSearch = ({
  notionToken,
}: { notionToken: string | null }) =>
  tool({
    description:
      'Search Notion pages and databases by title. Requires a Notion API integration token to be configured by the user.',
    parameters: z.object({
      query: z
        .string()
    ...
    execute: async ({ query, sort, filter, start_cursor, page_size }) => {
      if (!notionToken) {
        return {
          error:
